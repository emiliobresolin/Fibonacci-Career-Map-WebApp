...
- **Promotion commit** (single txn): **re-verify all four gates at commit time** — (a) latest snapshot `promotion_eligible = true`, (b) `organization.promotion_mode = ACTIVE`, (c) no open `calibration_flags` for the employee (`state = 'OPEN'`), (d) the approval-chain state is the terminal `APPROVED` per the configured workflow — rejecting with the same structured error codes used at Recommendation (`PROMOTION_NOT_ELIGIBLE`, `ORG_IN_CALIBRATION_MODE`, `CALIBRATION_FLAG_OPEN`, `APPROVAL_CHAIN_INCOMPLETE`) if any condition fails (the workflow may have lasted hours or days; eligibility can have lapsed). Then: insert PromotionRecord → update Employee level → reset level-scoped score inputs → insert outbox rows (audit + notification + 3D refresh). Commit. A commit-time rejection writes its own audit event capturing which gate failed and reverts the approval chain to a `REJECTED_AT_COMMIT` state so HR can investigate.
...
| `employee_assignments` | FK to employee + organization + role (`EMPLOYEE` \| `MANAGER` \| `ADMIN`); FK to `manager_employee_id` nullable (self-FK to `employees`); `assigned_at`; `deactivated_at` nullable; org-scoped uniqueness on `(employee_id, organization_id, role)` |
| `employee_blockers` | FK to employee_id (org-scoped via RLS); `kind ENUM('PIP','PERFORMANCE_CONCERN','HR_HOLD','OTHER')`; `reason TEXT NOT NULL CHECK (char_length(reason) >= 20)`; `opened_at`, `resolved_at` nullable; `opened_by`, `resolved_by` (FK to user); partial unique index `(employee_id, kind) WHERE resolved_at IS NULL`; the eligibility evaluator reads `EXISTS(... WHERE resolved_at IS NULL)` as the canonical blocker check for PRD §7.5 condition 4 |
...
- **No server-side caching of `/map/employees` or `/map/projection`.** Both endpoints are computed per-request from the live snapshot view + visibility-rule evaluation against the authenticated viewer. They MUST NOT be placed behind a shared cache (CDN, Redis cache layer, NestJS `CacheInterceptor`, or any reverse proxy with response caching), because the response is a function of `(viewer_user_id, viewer_role, visibility_scope, promotion_mode, latest_snapshots)` — any cache that omits even one of those dimensions risks cross-viewer payload poisoning (privacy incident: an `OWN_ONLY` viewer receiving an `ORG_FULL` viewer's response). Response headers explicitly set `Cache-Control: no-store, private` and `Vary: Authorization`. Per-request memoization within a single handler invocation is allowed; cross-request caching is forbidden. EPIC-16's cross-tenant regression suite includes a cross-viewer assertion within a single org to catch this regression.
- **Pinned `eligibility_state` enum** (single source of truth across server, client, 3D shaders, and `domain-contracts`): `'ELIGIBLE' | 'NOT_ELIGIBLE' | 'PENDING_CALIBRATION' | 'CALIBRATION_HOLD'`. The override hierarchy applied server-side is exactly `Rollout Mode > Calibration Hold > Eligibility`. Consumers must not invent additional values; if a fifth state ever becomes necessary, it is added here and propagated via a contracts version bump, not via ad-hoc enum extension at the consumer.
