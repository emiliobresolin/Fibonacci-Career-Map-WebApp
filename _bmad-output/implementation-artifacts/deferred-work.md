...
## Deferred from: Story 8-4 review (approve/reject + ApprovalRecord, 2026-05-23)

- **F8-4a (real scoring delta in `evidence.approved` audit payload).** Today the audit payload carries `beforeScore: 0` + `afterScore: 0` placeholders because Epic 8 lands before the scoring core. Epic 9's bulk-recalc consumer picks up the audit row via `originatingEventId` and produces the real `score.recalculated` snapshot delta, but the `evidence.approved` row itself is wrong-by-zero. Either (a) tighten the audit-payload emit once Epic 9 ships with the real before/after snapshot reads, or (b) relax the schema to make `beforeScore`/`afterScore` optional + omit them in 8-4 emits. Audit consumers should treat the current placeholders as "not yet meaningful" — flag in any 8-4-era audit dashboards that build off `evidence.approved.after.afterScore`.
- **F8-4b (recalc-enqueue failure repair signal).** When the BullMQ enqueue fails (Redis down), the approval state + audit are committed but no recalc fires. Today the service logs and continues — no Prom metric, no DB marker, no outbox row, so a future repair job has nothing deterministic to scan for. Lands as part of the Epic-9 scoring infrastructure: add a `pending_recalcs` table (or a counter metric `fcm_evidence_recalc_enqueue_failed_total`) populated when the enqueue throws, and a cron job that drains it.
- **F8-4c (Story 8-6 retroactive-rejection endpoint).** The PATCH /v1/evidence/:id/reject endpoint now returns 409 `use_retroactive_reject_endpoint` when the source state is APPROVED, signaling that retroactive rejection (FR-4.7) is a separate code path. Story 8-6 ships that path with score recalc + appropriate audit.
